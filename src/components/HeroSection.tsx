...
export default function HeroSection({ data, themeColor, template }: any) {
  ...
  const isResearcher = template === "Researcher";
  ...
  return (
    <section className="min-h-[80vh] flex flex-col justify-center py-20 scroll-mt-32">
      ...
      >
        ...
        {data.avatar && (
          <div className={`relative shrink-0 ${isResearcher ? 'p-1 border border-dashed border-gray-700' : ''}`}>
            <div className={`w-32 h-32 md:w-44 md:h-44 overflow-hidden ${isMinimalist ? 'rounded-full' : 'rounded-none border-2 border-white/10'}`}>
              <img src={data.avatar} alt="Srujan" className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-500" />
            </div>
            {isResearcher && <div className={`absolute -bottom-2 -right-2 w-4 h-4 border-r-2 border-b-2 ${activeAccent.replace('text', 'border')}`} />}
          </div>
        )}

        <div className="flex-grow">
          <p className={`font-mono text-xs uppercase tracking-[0.4em] mb-4 ${activeAccent}`}>
            {isResearcher ? "System Online // Initializing" : "Introduction"}
          </p>
          ...
          <div className={`text-2xl md:text-4xl mb-8 min-h-[1.5em] ${isMinimalist ? 'font-light text-gray-400' : 'font-mono opacity-80'}`}>
            {rolesArray.length > 0 ? (
              <TypeAnimation
                ...
                speed={50}
                repeat={Infinity}
                cursor={isResearcher} // Shows the blinking cursor in Researcher mode
              />
            ) : (
            ...
            {data.tags && data.tags.split(',').map((tag: string, index: number) => (
              <span 
                ...
                className={`px-4 py-1 text-[10px] font-mono uppercase tracking-widest transition-all ${
                  isResearcher ? `border border-gray-800 bg-black text-gray-400 hover:${activeAccent}` : 

...
export default function IdentitySection({ id, data, themeColor, template }: any) {
  ...
  return (
    <section id={id} className="scroll-mt-32">
      <motion.div
        initial={{ opacity: 0, x: isResearcher ? -20 : 0 }}
        whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.8 }}
      >
        {/* Heading Style Changes per Template */}
        <h3 className={`text-sm tracking-[0.3em] uppercase mb-12 flex items-center gap-4 ${isResearcher ? activeAccent : 'text-gray-400'}`}>
          {isResearcher && <span className="w-8 h-[1px] bg-current" />}
          {data.title}
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-start">
          ...
          <div className="md:col-span-4 flex flex-col gap-4">
            ...
            <div className={`p-6 ${
              isResearcher ? 'border border-gray-800 bg-black/40' : 
              ...
              <p className={`text-4xl font-bold ${isResearcher ? activeAccent : ''}`}>{data.stat_projects}</p>
            </div>
            ...
            <div className={`p-6 ${
              ...
              <p className={`text-xl font-bold ${isResearcher ? activeAccent : ''}`}>{data.stat_leadership}</p>
            </div>
          </div>
        </div>
      </motion.div>
